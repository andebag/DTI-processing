...
import dipy.reconst.dki_micro as dki
...
plt.imshow(data[:, :, axial_slice, 0].T, cmap='gray', origin='lower')
plt.subplot(1, 2, 2).set_axis_off()
plt.imshow(data[:, :, axial_slice, axial_slice].T, cmap='gray', origin='lower')
plt.show()

#loading in bvals and bvecs
...
#displaying gradient_table info
print(gtab.info)
print(gtab.bvals)
print(gtab.bvecs[:, :])

#part of DTI data that are S0
S0s = data[:, :, :, gtab.b0s_mask]
print(S0s.shape)

# nib.save(nib.Nifti1Image(S0s, img.affine), 'S0s.nii')
